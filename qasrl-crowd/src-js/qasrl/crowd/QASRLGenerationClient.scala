...
class QASRLGenerationClient[SID: Reader: Writer](instructions: VdomTag)(
  ...
  @Lenses case class QAPair(question: String, answers: List[Span], state: QAState)
                    ...
                    if (qaState.isComplete) {
                      ...
                      autocompleteResultOpt.foldMap {
                        ...
                        case Autocomplete.Incomplete(suggestions, badStartIndexOpt) =>
                          ...
                          def next =
                            setHighlightedIndex((highlightedIndex + 1) % menuItems.size.toInt)
                          def prev =
                            setHighlightedIndex(
                              (highlightedIndex - 1 + menuItems.size.toInt) % menuItems.size.toInt
                            )
                          CallbackOption.keyCodeSwitch(e) {
                            case KeyCode.Down => next >> e.preventDefaultCB
                            case KeyCode.Up   => prev >> e.preventDefaultCB
                            case KeyCode.Enter =>
                              selectItem(menuItems.toList(highlightedIndex)) >> e.preventDefaultCB
                          }
                      }
                    }
                ...
                <.div(
                  ...
                  (for {
                    ...
                    def itemBgStyle(tokenIndex: Int, isCompletion: Boolean) =
                      if (tokenIndex == highlightedIndex && isCompletion)
                        Some(^.backgroundColor := "rgba(0, 255, 0, 0.7)")
                      else if (tokenIndex == highlightedIndex)
            ...
            case Loaded(
              ...
              SpanHighlighting(
                SpanHighlightingProps(
                  ...
                  render = {
                    case (
                        SpanHighlightingState(spans, status),
                        SpanHighlightingContext(setSpan, hover, touch, cancelHighlight)
                        ) =>
