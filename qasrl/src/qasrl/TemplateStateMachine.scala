...
class TemplateStateMachine(
  ...
  val prepositionBigrams = lowerTokens
    .sliding(2)
  ...
  val allChosenPrepositions: Set[LowerCaseString] =
    overridePrepositions.getOrElse(
      ...
      prepositionBigrams ++
      TemplateStateMachine.mostCommonPrepositions.iterator).toSet
    )

  val nonToEndingPrepositions = NonEmptyList
    .of(
  ...
  val toEndingPrepositions = NonEmptyList
    .of(
      "to".lowerCase,
  ...
  )(wh: LowerCaseString): StateT[Option, FrameState, Argument] =
  ...
  val prepObjOpt = progress(
    ...
    " doing" -> (for { // asking about VP argument to prep
      fs          <- get
      preposition <- lift(fs.preposition)
      _ <- markAnswerSlot(
        Obj2,
        _ => pure[Argument](Prep((preposition.toString + " doing").lowerCase, Some(Noun(false))))
      )
    } yield qMark),
    " doing something" -> (for { // prep has inanimate placeholder VP argument
      fs          <- get
      preposition <- lift(fs.preposition)
      _ <- markPlaceholderSlot(
        Obj2,
        Prep((preposition.toString + " doing").lowerCase, Some(Noun(false)))
      )
    } yield qMark)
  )
  ...
  val postToObj = progress(
    ...
    " do" -> (for { // asking about VP argument to "to"
      fs          <- get
      preposition <- lift(fs.preposition)
      _ <- markAnswerSlot(
        Obj2,
        _ => pure[Argument](Prep((preposition.toString + " do").lowerCase, Some(Noun(false))))
      )
    } yield qMark),
    " do something" -> (for { // prep has inanimate placeholder VP argument
      fs          <- get
      preposition <- lift(fs.preposition)
      _ <- markPlaceholderSlot(
        Obj2,
        Prep((preposition.toString + " do").lowerCase, Some(Noun(false)))
      )
    } yield qMark)
  )
  ...
  val wh = {
    ...
    progress(
      ...
      "How much" -> modify(FrameState.whWord.set(Some("how much".lowerCase)))
        .as(auxRequiringSubject),
      "How long" -> modify(FrameState.whWord.set(Some("how long".lowerCase)))
        .as(auxRequiringSubject)
    )
  }
