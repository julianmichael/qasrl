...
object Dispatcher extends QASRLDispatcher[SentenceId] {
  ...
  val generationOverview = <.div(
    <.p(
      Styles.badRed,
      """Read through all of the instructions and make sure you understand the interface controls before beginning. A full understanding of the requirements will help make sure validators approve your work and you can retain your qualification."""
    ),
    ...
    <.p(
      """You will write questions about the verb and highlight their answers in the original sentence. """,
      <.b(
        """ Note: it takes exactly 2 clicks to highlight each answer; see the Interface & Controls tab for details. """
      ),
    ...
    <.p(
      "Occasionally, you may get a bolded word that isn't a verb, or is hard or impossible to write questions about. ",
  ...
  val generationQuestionFormat = <.div(
    <.p(
      """ This section is just for reference to help you understand the format of the questions.
  ...
  val generationControls = <.div(
    ...
    <.p(
      ...
      """. If it violates the required formatting, it will turn """,
      <.span(
        ^.backgroundColor := "rgba(255, 0, 0, 0.3)",
        "red"
      ),
      """. If it is a repeat of a previous question, it will turn """,
      <.span(
  ...
  val generationConditions = <.div(
    <.p(s"""Each question-answer pair after the first will earn you a bonus:
          ${dollarsToCents(generationReward)}c for the second question, ${dollarsToCents(
      generationReward
    ) + 1}c for the third
          ...
          """),
    <.p(
      """Your questions will be evaluated by other annotators, and """,
